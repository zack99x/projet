defmodule ProjectTimeManager.Management do
  @moduledoc """
  The Management context.
  """

  import Ecto.Query, warn: false
  alias ProjectTimeManager.Repo
  ...
  def getuser_by(email, username) do
    User
    |> where(email: ^email)
  ...
  def get_clock_by_user(user_id) do
    Repo.all(from(u in Clock, where: u.user_id == ^user_id))
  end
  ...
  # def get_wt_by_user(user_id) do
  #   Repo.all(from(wk in WorkingTime, where: wk.user_id == ^user_id))
  # end
  ...
  def get_wt_by_user_wt(user_id, id) do
    WorkingTime
    |> where(user_id: ^user_id)
    |> where(id: ^id)
    # |> preload(:user)
  ...
  def get_wt_by_user_start_end(user_id, starttime, endtime) do
    WorkingTime
    |> where(user_id: ^user_id)
